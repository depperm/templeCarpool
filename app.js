        ...
        var cursor=db.collection('Trips').find({$or:[pquery,fquery,dquery]}).toArray(function(err, results) {
            ...
            if(results!=''){
                ...
                if(req.body.email==results[0]['email']){
                    ...
                    res.send('You already have a seat reserved. If you\'d like to reserve more than one email the driver and have them reduce the number of seats on the trip or have your friend log in and make a reservation.')
                }
            }else{
                db.collection('Trips').update({'_id':ObjectId(req.params.trip)},{ $push: { "passengers": req.body }},function(err,res){
                    ...
                    checkEmailPref(req.body.email,'passJoin').then(function(shouldEmail){
                        if(shouldEmail){
                            console.log('should send')
                            sendEmail(trip.email,'You have joined a trip from '+trip.dDate+'-'+trip.rDate+'. Please visit templecarpool.com to change your email preferences or to leave the Trip.')
                        }else{
                            console.log('should not send')
                        }
                    })
                    //check if driver gets email
                    checkEmailPref(trip.email,'joinTrip').then(function(shouldEmail){
                        if(shouldEmail){
                            console.log('should send')
                            sendEmail(req.body.email,req.body.name+' has joined your trip from '+trip.dDate+'-'+trip.rDate+'. Please visit templecarpool.com to change your email preferences or to modify your Trip.')
                        }else{
                            console.log('should not send')
                        }
                    })
...
app.post('/api/trips/edit/:trip',function(req,res){
    ...
    db.collection('Trips').update({'_id':ObjectId(req.params.trip)},{$set:(req.body)},function(err,result){
        ...
        //TODO check if driver gets email
        /*checkEmailPref(req.body.email,'modifyYourTrip').then(function(shouldEmail){
            if(shouldEmail){
                console.log('should send')
                sendEmail(req.body.email,'You have modified your trip from '+req.body.dDate+'-'+req.body.rDate+'. Please visit templecarpool.com to change your email preferences or to modify your Trip.')
            }else{
                console.log('should not send')
            }
        })*/
...
app.delete('/api/trips/:trip',function(req,res){
    ...
    db.collection('Trips').deleteOne({'_id':ObjectId(req.params.trip)},function(err,result){
        ...
        //check if driver gets email
        checkEmailPref(yourEmail,'deleteTrip').then(function(shouldEmail){
            if(shouldEmail){
                console.log('should send')
                sendEmail(yourEmail,'You have deleted your trip on '+req.body.dDate+'-'+req.body.rDate+'. Please visit templecarpool.com to change your email preferences or to create a new Trip.')
            }else{
                console.log('should not send')
            }
        })
        ...
        for(let i=0;i<passengers.length;i++){
            checkEmailPref(passengers[i].email,'tripDeleted').then(function(shouldEmail){
                if(shouldEmail){
                    console.log('should send')
                    sendEmail(passengers[i].email,'The driver has deleted the trip you were on from '+req.body.dDate+'-'+req.body.rDate+'. Please visit templecarpool.com to change your email preferences or to find a new Trip.')
                }else{
                    console.log('should not send')
                }
            })
